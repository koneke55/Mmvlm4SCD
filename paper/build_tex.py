...
def build():
    ...
    a(r"\subsection{Future work}")
    a("Planned extensions: (1) external validation on dbGaP and MIMIC-"
      "IV-SCD subsets; (2) replacing the imaging encoder with a "
      "ResNet-/ConvNeXt-based CNN trained on erythrocytesIDB and the "
      "Kaggle Sickle RBC dataset; (3) ancestry-stratified loss "
      "weighting; (4) calibration improvement via temperature scaling "
      "and label-smoothing; (5) treatment-conditioned counterfactual "
      "risk under hydroxyurea / chronic transfusion programmes.")
